...
Solution:
Solution 1 is my solution, although it seems like using ROUND might be inaccurate. Solution 2 is from the solutions and calculates the MAX of a running total using a variable @c.
*/
...
BEGIN
    SET @c := 1;
    SELECT 
        MAX(@c := @c * LENGTH(characters)) AS combinations
    FROM discs;
END